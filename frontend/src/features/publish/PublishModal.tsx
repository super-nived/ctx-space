...
export function PublishModal({ projectId, projectName, defaultCommitMessage = '', onClose }: PublishModalProps) {
  ...
  const publish = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/projects/${projectId}/publish`, {
        ...
        body: JSON.stringify({ commit_message: commitMessage.trim() || null }),
      });
      if (!res.ok) {
    ...
    <>
      ...
      >
        ...
        {phase === 'idle' && (
          <>
            <div
              className="mb-4 flex items-start gap-3 rounded-xl border p-4 text-[13px]"
            ...
            >
              ...
              <div>
                All generated files will be pushed to a new (or existing) repository on your GitHub account. The repo will be public.
              </div>
            </div>
            ...
            <div className="mb-4">
              ...
              <input
                ...
                placeholder={`Update ${projectName} via Context Space`}
                maxLength={72}
                className="w-full rounded-xl border bg-transparent px-3 py-2 text-[13px] outline-none focus:ring-1 focus:ring-indigo-500"
