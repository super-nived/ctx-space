...
function ProjectNameEditor() {
  ...
  if (editing) {
    return (
      <input
        ...
        className="text-ink bg-surface-2 w-40 rounded-md px-2 py-0.5 text-sm font-medium outline-none ring-2 ring-indigo-400"
        maxLength={60}
        autoFocus
      />
    );
  ...
  return (
    ...
    >
      ...
      <svg className="text-ink-muted h-3 w-3 opacity-0 group-hover:opacity-100" viewBox="0 0 16 16" fill="currentColor">
...
export function TopBar({ onToggleMobileView, mobileView, onOpenHistory }: TopBarProps) {
  ...
  const newProject = () => {
    if (
      !confirm(
        'Start a new project? The current one is saved and will appear in your History.',
      )
    ) {
      return;
    }
    ...
    <header className="border-border-subtle bg-canvas flex h-12 shrink-0 items-center justify-between border-b px-3">
      ...
      <div className="flex items-center gap-1">
        ...
        <button
          ...
          className="text-ink-soft hover:bg-surface-2 flex items-center gap-1 rounded-md px-2.5 py-1 text-[13px]"
        >
